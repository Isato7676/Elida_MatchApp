  ...
  try {
    ...
    // 4. RUTAS ESTÁNDAR DE LA RFFM (CALENDARIOS Y APIS DIRECTAS)
        if (endpoint === 'calendario-next') {
          // Mapeamos los parámetros para incluir los nombres que exige la RFFM (id_grupo, id_competicion, id_temporada)
          const params = new URLSearchParams(queryParams);
          
          if (queryParams.grupo && !params.has('id_grupo')) {
            params.append('id_grupo', queryParams.grupo);
          }
          if (queryParams.competicion && !params.has('id_competicion')) {
            params.append('id_competicion', queryParams.competicion);
          }
          if (queryParams.temporada && !params.has('id_temporada')) {
            params.append('id_temporada', queryParams.temporada);
          }
    
          // Petición dinámica a la página pública del calendario con los parámetros adaptados
          const targetUrl = `https://www.rffm.es/competicion/calendario?${params.toString()}`;
